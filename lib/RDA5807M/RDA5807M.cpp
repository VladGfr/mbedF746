...
//Vérifie que le module est présent sur le bus
bool RDA5807M::verifCo() {
        if(i2c.write(0x10 << 1, NULL, 0) || i2c.write(0x11 << 1, NULL, 0)){
            return true;
        } else {
            return false;
        }
...
void RDA5807M::SetThreshold(int threshold) { //Threshold seekup/seekdown => Registre 0x05, bits 11:8
    RDA5807M_WriteReg[4] = (RDA5807M_WriteReg[4] & 0xF0FF) | (threshold << 8);
    WriteAll();
}
